...
###################################################3
            ...
            # Temperature (gas)
            Cov_Cpg = np.zeros(N) # Heat capacity (overall) J/K/m^3
            dCov_Cpg_D = np.zeros(N)
            for ii in range(n_comp):
                Cov_Cpg = Cov_Cpg + Cpg[ii]*C[ii]
                dCov_Cpg_D = dCov_Cpg_D + D_dis[ii]*Cpg[ii]*dC[ii]
            
            dTgdt = -v*dTg + 1/Cov_Cpg*(dCov_Cpg_D*dTg +h_heat*a_surf/epsi*(Ts - Tg) )
            dTgdt[0] = h_heat*a_surf/epsi*(Ts[0] - Tg[0])/Cov_Cpg[0]
            #dTgdt[0] = dTgdt[0] + (v_in*self._T_in*Cov_Cpg_in/Cov_Cpg[0] - v[0]*Tg[0])/h
            dTgdt[0] = dTgdt[0] + v_in*Cov_Cpg_in/Cov_Cpg[0]*(self._T_in - Tg[0])/h
            #dTgdt[0] = dTgdt[0] + dCov_Cpg_D[0]/Cov_Cpg[0]*(Tg[0] - self._T_in)/h

            dTgdt[-1] = h_heat*a_surf/epsi*(Ts[-1] - Tg[-1])/Cov_Cpg[-1]
            dTgdt[-1] = dTgdt[-1] + v[-2]*Cov_Cpg[-2]/Cov_Cpg[-1]*(Tg[-2] - Tg[-1])/h
            #dTgdt[-1] = dTgdt[-1] + dCov_Cpg_D[-1]/Cov_Cpg[-1]*(Tg[-1] - Tg[-2])/h
